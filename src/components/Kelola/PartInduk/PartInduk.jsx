...
const PartInduk = () => {
  ...
  const handleEdit = async (values) => {
    try {
      const response = await axios.patch("/api/anggaran/ubah", {
        id_anggaran: selectedAnggaran.id_anggaran,
        namaAnggaran: values.nama,
        jumlahAnggaran: values.jumlah
          ? parseInt(values.jumlah.replace(/\./g, ""), 10)
          : 0,
        tanggalAwal: dayjs(values.periode[0]).format("DD-MM-YYYY"),
        tanggalAkhir: dayjs(values.periode[1]).format("DD-MM-YYYY"),
        statusAnggaran: values.status === true ? "Aktif" : "Nonaktif",
      ...
      if (response.status === 200) {
        notification.success({
          message: "Berhasil",
          description: "Anggaran berhasil diubah",
          placement: "top",
          duration: 5,
        });
      } else {
        notification.error({
          ...
          description: "Anggaran gagal diubah",
          placement: "top",
          duration: 5,
        });
      }
  ...
  const handleSubmit = async (values) => {
    try {
      const response = await axios.post("/api/anggaran/tambah", {
        namaAnggaran: values.nama,
        jumlahAnggaran: values.jumlah
          ? parseInt(values.jumlah.replace(/\./g, ""), 10)
          : 0,
      ...
      if (response.status === 200) {
        ...
        notification.error({
          message: "Error",
          description: "Terjadi kesalahan saat menambah anggaran",
          placement: "top",
          duration: 3,
        });
      }
  ...
  const handleDelete = async (values) => {
    onDetailDrawerClose();
    try {
      const response = await axios.post("/api/anggaran/hapus", {
        id_anggaran: values,
      });
      const data = response.data.rows;
  ...
  const fetchTransaksiAnggaran = async (id) => {
    try {
      const response = await axios.post("/api/anggaran/fetch", {
        id_anggaran: id,
      });
      ...
      if (!listData) {
        throw new Error("Data rows is undefined");
      }

      const mappedData = listData.map((transaksi, index) => {
        const options = {
          weekday: "long",
          year: "numeric",
          month: "long",
        ...
        const formattedDate = new Date(transaksi.tanggal).toLocaleDateString(
          "id-ID",
          options
        );

        return {
          key: transaksi.id_kas,
          no: index + 1,
          nama: transaksi.nama,
          tanggal: formattedDate,
          kategori: transaksi.nama_kategori,
          jumlah: "Rp " + parseInt(transaksi.jumlah).toLocaleString("id-ID"),
        };
      });

      setTransaksiAnggaran(mappedData);
    } catch (error) {
      console.error("Error fetching anggaran:", error);
    }
  };

  const fetchArusKasAnggaran = async () => {
    try {
      const response = await axios.get("/api/anggaran/fetch");
    } catch (error) {
      console.error("Error fetching anggaran:", error);
    }
  };
